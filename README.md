...
QQQ ETF의 200일 단순이동평균선(SMA) 크로스오버를 자동 감지하고, 시그널 변경 시 Discord로 알림을 보내는 봇입니다.
...
1. 매일 평일(KST 07:00) GitHub Actions가 자동 실행
2. Yahoo Finance에서 QQQ 가격 데이터를 가져와 200일 SMA 계산
3. 이전 시그널과 비교하여 변경 시에만 Discord 알림 전송
4. 매월 1일에는 현재 상태 요약 리포트 전송
