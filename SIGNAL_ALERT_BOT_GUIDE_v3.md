# H 전략 V16a_c20_VIX35 시그널 알림 봇 가이드 v3

**BlackRabbit LAB · v3 · 2026-04-27**

**F2D3 + GCE + PRE(VIX35) 기준 · Phase 1 자동 알림 시스템 · 월말 시점 공식화**

---

## v2 → v3 핵심 변경

- **PRE VIX 임계치 완화: 40 → 35** (V16a_c20 → V16a_c20_VIX35)
- **월말 시점 공식화**: 시뮬 = 실전 운용 시점 일치 (교훈 #44). 봇은 이미 월말 체크 구현 → 시뮬과 일치 확인 완료
- **V16d_CRE 미포함**: CRE(SMA50<SMA200 시 월말 On 차단)는 Bias Audit p=0.24로 fragile alpha 판정, 채택 철회 (교훈 #45). 봇에 CRE 로직 불필요
- **workflow 명 갱신**: H v5 → H v6
...
H 전략 Phase 1 V16a_c20_VIX35의 매매 시그널을 자동 감지하고 알림을 발송하는 봇.
...
### 2.1 Phase 1 V16a_c20_VIX35 상태 체계
...
```python
"""H 전략 V16a_c20_VIX35 시그널 체커 — F2D3 + GCE + PRE(VIX35)."""
...
GCE_DEVIATION = 0.01
PRE_VIX_THRESHOLD = 35.0       # ★ v3: 40 → 35
PRE_VIX_DROP_THRESHOLD = 5.0
PRE_VIX_DROP_LOOKBACK = 5
PRE_COOLDOWN = 60               # 거래일 (PRE 재발동 쿨다운)
PRE_MONTHLY_COOLDOWN = 20       # 일 (PRE 진입 후 월말 체크 비활성)
...
def main():
    print(f"=== H 전략 V16a_c20_VIX35 시그널 체크 ({datetime.now():%Y-%m-%d %H:%M}) ===\n")
...
name: H v6 Signal Check (V16a_c20_VIX35)
on:
  schedule:
...
```
═══════════════════════════════════════════
  H 전략 V16a_VIX35 일일 리포트 (2026-04-27)
═══════════════════════════════════════════
  상태: 🟢 On [PRE 보유 아님]
...
이 기록은 **out-of-sample 검증**에 활용. 실전 배포 후 첫 PRE 이벤트는 V16a_c20_VIX35의 real-world 검증 핵심 데이터.

---

## 6. v2 → v3 코드 마이그레이션

기존 v2 봇 사용자는 다음 한 줄만 변경:
...
그 외 모든 로직(Exit, GCE, PRE 쿨다운, 가격 Exit, 자동 해제, 월말 체크)은 동일. CRE 로직은 v2에도 없었으므로 변경 불필요.
...
*BlackRabbit LAB · SIGNAL_ALERT_BOT_GUIDE v3 · 2026-04-27*
*H 전략 Phase 1 V16a_c20_VIX35: F2D3 + GCE + PRE(VIX35) 기준*
